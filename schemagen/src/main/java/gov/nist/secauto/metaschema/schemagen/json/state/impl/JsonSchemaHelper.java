...
public final class JsonSchemaHelper {
  private static final Comparator<IFlagInstance> FLAG_INSTANCE_QNAME_COMPARATOR
      = Comparator.comparing(instance -> instance.getQName());
  private static final Comparator<IModelInstanceAbsolute> MODEL_INSTANCE_NAME_COMPARATOR
      = Comparator.comparing(instance -> getJsonPropertyName(ObjectUtils.notNull(instance)));
  ...
  @NonNull
  private static String getJsonPropertyName(@NonNull IInstanceAbsolute instance) {
    String retval;
    if (instance instanceof IFlagInstance) {
      retval = ((IFlagInstance) instance).getJsonName();
    } else if (instance instanceof INamedModelInstance) {
      retval = ((INamedModelInstance) instance).getJsonName();
    } else if (instance instanceof IChoiceGroupInstance) {
      retval = ((IChoiceGroupInstance) instance).getJsonName();
    } else {
      throw new UnsupportedOperationException("Unsupported model instance type: " + instance.getClass());
    }
    return retval;
  }
  ...
  public static Stream<Choice> explodeChoices(
    ...
    for (IChoiceInstance choice : choiceInstances) {
      List<IJsonSchemaPropertyNamed> newChoices = JsonSchemaHelper.buildModelProperties(choice, state);
